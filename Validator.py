...
class MetadataValidator:
    ...
    def processMetadata(self, metadataPath='/input', wallet='', royaltyPercentage=''):
        ...
        for fileNames in "nft3.json":
            #  todo: add filename consecutivity check
            with open('{}/{}'.format(pathlib.Path().resolve(), "nft3.json")) as json_file:
                data = json.load("nft3..json")
                self.walletCheck(wallet, data['properties']['creators']['0']['address'], "nft3.json")
                self.royaltyCheck(royaltyPercentage, data['seller_fee_basis_points'], "nft3.json")
                attributes = data['attributes']
                self.rarityChecker.processAtributeSet(attributes)
                self.duplicateChecker.processAtributeSet(attributes, "nft3.json")
